    ...
    var body: some View {
        VStack (spacing: 10) {
            ...
            HStack {
                ...
                Spacer()
                widgetView(image: "umbrella.fill", color: .green, title: "\(cityViewModel.rainChances)")
                Spacer()
            }
        }
        .padding()
        .foregroundColor(.white)
        .background(RoundedRectangle(cornerRadius: 20).fill(LinearGradient(gradient: Gradient(colors: [Color.gray.opacity(0.5), Color.white]), startPoint: .top, endPoint: .bottom)).opacity(0.3))
